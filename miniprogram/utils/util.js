 ...
 * 格式化价格数额为字符串
 * 可对小数部分进行填充，默认不填充
 * @param price 价格数额，单位：元 (原逻辑为分，现统一为元)
...
function priceFormat(price, fill = 0) {
  ...
  if (fill > 0) {
    ...
    const decimalPart = priceFormatValue.split('.')[1];
    const n = fill - (decimalPart ? decimalPart.length : 0);
    for (let i = 0; i < n; i++) {
...
const generateId = (prefix) => {
  ...
  // 用户部分：取OpenID后2位，若无则随机2位
  const userPart = openId
    ? openId.slice(-2).toUpperCase()
    : Math.random().toString(36).substr(2, 2).toUpperCase();

  return `${prefix}-${timePart}${userPart}`;
};
